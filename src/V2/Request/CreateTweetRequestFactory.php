...
final class CreateTweetRequestFactory
{
    public function __construct(
        private readonly RequestFactoryInterface $requestFactory,
        private readonly StreamFactoryInterface $streamFactory,
        private readonly TwitterApiConfig $config,
        private readonly CreateTweetParams $params,
    ) {}

    public function create(): RequestInterface
    {
        $url = $this->config->baseUrl . '/2/tweets';
        $body = $this->streamFactory->createStream(
            json_encode($this->params->toArray(), JSON_THROW_ON_ERROR),
        );
