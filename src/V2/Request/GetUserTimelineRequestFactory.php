...
final class GetUserTimelineRequestFactory
{
    public function __construct(
        private readonly RequestFactoryInterface $requestFactory,
        private readonly TwitterApiConfig $config,
        private readonly string $userId,
        private readonly ?UserTimelineParams $params = null,
        private readonly ?TweetFieldsParams $fields = null,
    ) {}

    public function create(): RequestInterface
    {
        $url = $this->config->baseUrl . '/2/users/' . $this->userId . '/tweets';
        $queryParams = array_merge(
            $this->params?->toQueryParams() ?? [],
            $this->fields?->toQueryParams() ?? [],
        );
        if ($queryParams !== []) {
